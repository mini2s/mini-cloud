...
// TestMaybeRetryFailedTask_WorkflowTaskWithoutIssueID verifies that a failed
// workflow node-run task with no IssueID (default-workflow runs create no
// sub-issue, so IssueID is NULL) is still auto-retried. Before the fix, the
// IssueID/ChatSessionID gate in maybeRetryFailedTask short-circuited retry for
// these tasks, so a timeout failure cancelled the whole workflow instead of
// re-dispatching the node.
func TestMaybeRetryFailedTask_WorkflowTaskWithoutIssueID(t *testing.T) {
	pool := openTestPool(t)
	defer pool.Close()
	...
	suffix := fmt.Sprintf("wf-retry-%d", os.Getpid())

	var workspaceID string
	if err := pool.QueryRow(ctx, `
		INSERT INTO multica_workspace (name, slug, description, issue_prefix)
		VALUES ($1, $2, 'workflow retry test workspace', 'WFRT')
		RETURNING id
	`, "Workflow Retry Workspace "+suffix, "wf-retry-"+suffix).Scan(&workspaceID); err != nil {
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ($1, 'Retry Run', 'retry run', 'active', 0, 'member', gen_random_uuid())
		RETURNING id
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ($1, 'dev-retry-1', 'test-runtime', 'local', 'test', 'online', '', '{}')
		RETURNING id
	...
	// Failed task: NO issue_id (default-workflow), workflow_node_run_id set,
	// failure_reason='timeout' (retryable), attempt=1 < max_attempts=3.
	var taskID string
	if err := pool.QueryRow(ctx, `
		INSERT INTO multica_agent_task_queue (
			agent_id, runtime_id, status, priority,
			workflow_node_run_id, context, error, failure_reason, attempt, max_attempts
		)
		VALUES ($1, $2, 'failed', 2, $3, $4, 'task timed out', 'timeout', 1, 3)
		RETURNING id
	...
	// Baseline dispatch job count for this node run (StartRun may enqueue one).
	var baselineJobs int
	if err := pool.QueryRow(ctx, `
		SELECT count(*) FROM multica_workflow_node_run_dispatch_job WHERE workflow_node_run_id = $1
	`, nodeRun.ID).Scan(&baselineJobs); err != nil {
		t.Fatalf("count baseline dispatch jobs: %v", err)
	}

	result, err := taskSvc.maybeRetryFailedTask(ctx, task)
	if err != nil {
		t.Fatalf("maybeRetryFailedTask: %v", err)
	}
	if !result.Scheduled {
		t.Fatalf("expected retry to be scheduled for workflow task without IssueID, got Scheduled=false")
	}

	// A new dispatch job must have been enqueued for the node run.
	var afterJobs int
	if err := pool.QueryRow(ctx, `
		SELECT count(*) FROM multica_workflow_node_run_dispatch_job WHERE workflow_node_run_id = $1
	`, nodeRun.ID).Scan(&afterJobs); err != nil {
		t.Fatalf("count dispatch jobs after retry: %v", err)
	}
	if afterJobs <= baselineJobs {
