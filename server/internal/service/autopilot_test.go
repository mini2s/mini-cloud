...
func TestBuildIssueDescription_NoTriggerPayload(t *testing.T) {
	...
	run := db.MulticaAutopilotRun{Source: "schedule", TriggeredAt: pgtype.Timestamptz{Time: time.Now().UTC(), Valid: true}}
...
func TestBuildIssueDescription_UsesTriggerTimezone(t *testing.T) {
	s := &AutopilotService{}
	ap := db.MulticaAutopilot{Description: pgtype.Text{String: "daily sync", Valid: true}}
	run := db.MulticaAutopilotRun{
		Source:      "schedule",
...
func TestBuildIssueDescription_WithWebhookPayload(t *testing.T) {
	...
	run := db.MulticaAutopilotRun{Source: "webhook", TriggerPayload: payload, TriggeredAt: pgtype.Timestamptz{Time: time.Now().UTC(), Valid: true}}
...
func TestBuildIssueDescription_NonWebhookSourceWithPayloadIgnored(t *testing.T) {
	...
	s := &AutopilotService{}
	ap := db.MulticaAutopilot{Description: pgtype.Text{String: "thing", Valid: true}}
	run := db.MulticaAutopilotRun{Source: "manual", TriggerPayload: []byte(`{"event":"x.y"}`), TriggeredAt: pgtype.Timestamptz{Time: time.Now().UTC(), Valid: true}}
...
func TestInterpolateTemplate(t *testing.T) {
	s := &AutopilotService{}
	run := db.MulticaAutopilotRun{TriggeredAt: pgtype.Timestamptz{Time: time.Now().UTC(), Valid: true}}
