...
-- task targets a multica_project_resource of type `local_directory` whose path is
...
ALTER TABLE multica_agent_task_queue DROP CONSTRAINT IF EXISTS agent_task_queue_status_check;
ALTER TABLE multica_agent_task_queue ADD CONSTRAINT agent_task_queue_status_check
    CHECK (status IN ('queued', 'dispatched', 'running', 'waiting_local_directory', 'completed', 'failed', 'cancelled'));

ALTER TABLE multica_agent_task_queue ADD COLUMN IF NOT EXISTS wait_reason TEXT;
