-- Lark (飞书) Bot integration: per-multica_agent PersonalAgent installations,
-- multica_user/chat bindings, inbound dedup + drop audit, outbound card mapping,
-- and short-lived multica_member binding tokens.
--
-- Scope notes (mirror description §4.8 boundaries):
--   * `multica_chat_session` is reused as-is — Lark routes through a separate
--     `multica_lark_chat_session_binding` rather than adding a `metadata` JSONB
--     column to multica_chat_session.
--   * Outbound card-message mapping is *task/message* scoped, not session
--     scoped, so multiple runs on the same multica_chat_session don't stomp each
--     other's cards.
...
--   * `multica_lark_inbound_audit` is the only writable surface for events that
...
-- =====================
-- One row per (multica_workspace, multica_agent) — each Multica Agent owns at most one
...
CREATE TABLE multica_lark_installation (
    ...
    installer_user_id     UUID NOT NULL REFERENCES "multica_user"(id) ON DELETE RESTRICT,
    ...
    UNIQUE (app_id),
    -- Composite key target for the composite FK on multica_lark_user_binding
    -- (installation_id, workspace_id) — guarantees a binding's multica_workspace
    -- always matches the multica_workspace of its installation.
    UNIQUE (id, workspace_id)
);

CREATE INDEX idx_lark_installation_workspace ON multica_lark_installation(workspace_id);
CREATE INDEX idx_lark_installation_agent ON multica_lark_installation(agent_id);
-- Used by the lease scanner to find leases due for renewal / takeover.
CREATE INDEX idx_lark_installation_lease ON multica_lark_installation(ws_lease_expires_at)
    WHERE status = 'active';

-- =====================
-- multica_lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a Multica multica_user, per-installation. open_id is
-- per-app, so the same Lark multica_user has different open_ids across different
...
-- in MVP.
--
-- Two structural invariants protect §4.3's "unbound or non-multica_workspace
-- members never leak content into multica_chat_session" rule from drifting if
-- the application layer regresses:
--
--   1. The composite FK on (installation_id, workspace_id) targets
--      multica_lark_installation(id, workspace_id), so a binding row cannot
--      claim a multica_workspace different from its installation's multica_workspace.
--
--   2. The composite FK on (workspace_id, multica_user_id) targets
--      multica_member(workspace_id, user_id) with ON DELETE CASCADE, so when a
--      Multica multica_user is removed from the multica_workspace the stale Lark
--      binding is removed in the same transaction. There is no path
--      where multica_lark_user_binding can outlive multica_workspace membership.
CREATE TABLE multica_lark_user_binding (
    id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    -- Installation ↔ multica_workspace integrity. Composite FK guarantees the
    ...
    CONSTRAINT lark_user_binding_installation_fk
        ...
        REFERENCES multica_lark_installation(id, workspace_id)
        ON DELETE CASCADE,
    -- Workspace membership integrity. Composite FK guarantees the
    -- (workspace_id, multica_user_id) pair still exists in multica_member; when
    -- the multica_user is removed from the multica_workspace, the binding cascades away.
    CONSTRAINT lark_user_binding_member_fk
        FOREIGN KEY (workspace_id, multica_user_id)
        REFERENCES multica_member(workspace_id, user_id)
        ON DELETE CASCADE
);

CREATE INDEX idx_lark_user_binding_user
    ON multica_lark_user_binding(multica_user_id, workspace_id);
CREATE INDEX idx_lark_user_binding_workspace_open
    ON multica_lark_user_binding(workspace_id, lark_open_id);

-- =====================
-- multica_lark_chat_session_binding
-- =====================
-- One Lark chat (`chat_id`) ↔ one Multica `multica_chat_session`. The Lark side
...
CREATE TABLE multica_lark_chat_session_binding (
    id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    chat_session_id   UUID NOT NULL REFERENCES multica_chat_session(id) ON DELETE CASCADE,
...
CREATE INDEX idx_lark_chat_session_binding_session
    ON multica_lark_chat_session_binding(chat_session_id);
...
--     persisted, OR multica_chat_message + session touched). If the worker
...
--           multica_chat_message+Mark transaction; the token-fenced Mark
--           returns zero rows because the live token is now T_B, so
--           A's tx ROLLS BACK and no second multica_chat_message lands. B's
--           run is the sole writer.
--
--       (2) Mark window. The dispatcher Marks the dedup row INSIDE
--           the same tx as the multica_chat_message + session touch, so the
--           durable write and the Mark commit (or roll back)
--           atomically. There is no "committed multica_chat_message but not
...
-- dropped (group message without @, unbound multica_user, non-multica_workspace multica_member,
...
-- proving the "non-bound users cannot leak content into multica_chat_session"
-- invariant from §4.7 of the design.
CREATE TABLE multica_lark_inbound_audit (
    id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    installation_id   UUID REFERENCES multica_lark_installation(id) ON DELETE SET NULL,
    lark_chat_id      TEXT,
...
CREATE INDEX idx_lark_inbound_audit_installation
    ON multica_lark_inbound_audit(installation_id, received_at DESC);
CREATE INDEX idx_lark_inbound_audit_reason
    ON multica_lark_inbound_audit(drop_reason, received_at DESC);
...
-- — a multica_chat_session can host many runs and a session-level field would
...
CREATE TABLE multica_lark_outbound_card_message (
    id                     UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    chat_session_id        UUID NOT NULL REFERENCES multica_chat_session(id) ON DELETE CASCADE,
    task_id                UUID REFERENCES multica_agent_task_queue(id) ON DELETE SET NULL,
    lark_chat_id           TEXT NOT NULL,
...
CREATE UNIQUE INDEX idx_lark_outbound_card_task
    ON multica_lark_outbound_card_message(task_id)
    WHERE task_id IS NOT NULL;
CREATE INDEX idx_lark_outbound_card_session
    ON multica_lark_outbound_card_message(chat_session_id, created_at DESC);

-- =====================
-- multica_lark_binding_token
-- =====================
-- Short-lived (≤ 15 min), single-use token for the "you're not bound yet,
-- click here" flow that links a Lark `open_id` to a Multica multica_user. The
...
CREATE TABLE multica_lark_binding_token (
...
CREATE INDEX idx_lark_binding_token_installation
    ON multica_lark_binding_token(installation_id, expires_at);
