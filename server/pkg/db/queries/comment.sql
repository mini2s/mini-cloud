...
-- Top-level comments only, in multica_issue chronological order, each annotated with
...
-- `multica_comment list --roots-only` so agents can not only orient around the global
...
-- those threads' subtrees instead of every thread in the multica_issue. membership
-- labels each multica_comment with its thread root by walking down from the selected
...
FROM selected_roots sr
JOIN multica_comment c ON c.id = sr.id
...
-- Counts comments on an multica_issue created strictly after @since, ACROSS THE WHOLE
-- ISSUE (every thread, not just the triggering one). Excludes the triggering
-- multica_comment itself (@anchor_id — its body is already injected into the prompt)
-- and any authored by the given multica_agent (@author_id), so a chatty multica_agent does not
-- inflate its own new-multica_comment count. The multica_agent is steered to read the
-- triggering thread first (see BuildNewCommentsHint), but the count is
-- multica_issue-wide so it knows the full catch-up volume. Feeds the daemon claim
-- response without shipping multica_comment bodies.
SELECT count(*) FROM multica_comment
WHERE issue_id = @issue_id
  AND workspace_id = @workspace_id
  AND created_at > @since
  AND id <> @anchor_id
  AND NOT (author_type = 'multica_agent' AND author_id = @author_id);
...
-- Returns the thread-root multica_comment for @comment_id by walking parent_id up to
-- the row whose parent_id IS NULL. For a root multica_comment it returns that multica_comment
...
-- multica_comment cannot be its own ancestor).
WITH RECURSIVE root_of AS (
    SELECT c.id, c.parent_id
    FROM multica_comment c
...
SELECT c.* FROM multica_comment c
...
WHERE parent_id = @parent_id AND author_type = 'multica_agent' AND author_id = @agent_id;
