...
--   1. PostgreSQL's FK validation on multica_agent.runtime_id (FK ... ON DELETE
--      RESTRICT) needs FOR KEY SHARE on the parent runtime row, which
--      conflicts with FOR UPDATE — so any concurrent INSERT or UPDATE
--      that would point a new/moved multica_agent at this runtime blocks until
...
WHERE status = 'active'
  AND assignee_type = 'multica_agent'
...
-- Removes archived squads whose leader_id references an archived multica_agent on the
-- given runtime. Must run before DeleteArchivedAgentsByRuntime so the RESTRICT
-- FK on multica_squad.leader_id does not block the multica_agent deletion. Active squads are
