...
-- Replaces an multica_agent's custom_env map wholesale. Used by the dedicated
...
-- is the authoritative bound on what gets archived: any multica_agent that
...
-- multica_agent:archived per multica_agent.
UPDATE multica_agent
SET archived_at = now(), archived_by = @archived_by, updated_at = now()
...
-- Returns every non-archived multica_agent bound to a runtime. Backs the cascade
...
-- each currently-active multica_agent row so a concurrent archive/move of one
...
-- add a new multica_agent to the runtime mid-cascade. Together they guarantee
...
-- Loads a task only when its owning multica_agent lives in the given multica_workspace.
-- agent_id is NOT NULL on every task row (and ON DELETE CASCADE, so the multica_agent
...
-- (multica_issue / multica_chat_session / multica_autopilot_run) happens to be set. It is what lets
-- run_only multica_autopilot tasks and quick_create tasks (whose multica_issue does not exist
...
-- multica_project_resource of type local_directory. wait_reason carries a short
...
-- Returns the started_at of the most recent prior task for this (multica_agent, multica_issue)
-- pair, used as the "since" anchor for counting comments that arrived since the
-- multica_agent's last run. Any terminal state counts as "a run happened". Tasks with
...
SELECT started_at FROM multica_agent_task_queue
...
-- or running task for the multica_issue.
