...
-- Aggregates the multica_issue's linked PRs into the two counts that gate
...
-- webhook auto-advances the multica_issue when open_count = 0 AND
...
-- PR has already merged still resolves the multica_issue.
SELECT
...
ON CONFLICT (issue_id, pull_request_id) DO UPDATE SET
    close_intent = CASE
        WHEN sqlc.arg('preserve_close_intent') THEN multica_issue_pull_request.close_intent
