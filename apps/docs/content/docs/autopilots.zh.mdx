---
title: Autopilots
description: 让数智人按 cron 定时自己开工，或在 webhook 到来时被触发——也可以通过 UI / CLI 手动触发一次。
---

import { Callout } from "fumadocs-ui/components/callout";

Autopilots 让 [数智人](/agents) **按调度自动开工**——配好 cron 和时区，到点 Multica 自己派发 [`task`](/tasks)，不需要你每次触发。适合定期巡检、周期性报告、凌晨跑的清理任务这类"standing order"场景。和前三种触发方式（[分配](/assigning-issues) / [@ 提及](/mentioning-agents) / [对话](/chat) 都是你主动喊一声）相比，Autopilots 的核心差别是**时间驱动**。
...
- **先建 issue 模式**（`create_issue`）—— 默认，**推荐**。每次触发先在工作区里建一个 issue（标题目前只支持一个占位符 `{{date}}`，会插值成 UTC 日期 `YYYY-MM-DD`；其他 `{{...}}` 形式的占位符会在创建时被拒绝，避免拼错以后悄无声息地把原文当成 issue 标题），再按分配流程把 issue 派给数智人。所有工作都落在 issue 看板上，历史、评论、状态和手动分配的 issue 完全一致。
...
在**先建 issue 模式**下，入站 payload 会附加在新 issue 的描述里供数智人
...
- `{"status":"skipped","run_id":"…","reason":"agent runtime is offline at dispatch time"}`
  —— 受派数智人的 runtime 离线，记为 `skipped` run。
...
如果 Autopilot 很重要，要自己设计监控——例如让数智人在成功时给自己发个评论，通过缺失评论来发现失败。
</Callout>
...
- [**分配 issue 给数智人**](/assigning-issues) —— 一次性把 issue 指派给数智人
- [**在评论里 @ 数智人**](/mentioning-agents) —— 评论里让数智人看一眼
