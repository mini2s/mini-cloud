...
Each user who wants to run AI digital humans locally also installs the **`cs-workflow` CLI** and runs the **digital human daemon** on their own machine.
...
<Callout>
If the self-host server is already running and you only need the CLI on a macOS/Linux machine, install it with the install script: `curl -fsSL https://raw.githubusercontent.com/Askhz/multica/main/scripts/install.sh | bash`.
</Callout>
...
The daemon runs on your local machine (not inside Docker). It detects installed AI digital human CLIs, registers them with the server, and executes tasks.

### a) Install the CLI and an AI digital human

```bash
curl -fsSL https://raw.githubusercontent.com/Askhz/multica/main/scripts/install.sh | bash
```

You also need at least one AI digital human CLI:
...
3. Go to **Settings → Digital Humans** and create a new digital human
4. Create an issue and assign it to your digital human
...
Digital Human-specific overrides:
