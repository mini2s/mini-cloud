---
title: Workspaces
description: A workspace is the self-contained space where a group collaborates — every issue, member, comment, and digital human belongs to one.
---

import { Callout } from "fumadocs-ui/components/callout";

A workspace is **the self-contained space where a group collaborates in Multica** — every [issue](/issues), [member](/members-roles), [comment](/comments), and [digital human](/agents) belongs to one. The issue list, member roster, and digital human configuration you see after logging in are all scoped to the current workspace; **switching workspaces replaces the entire view**.
...
- Every digital human configuration along with its task history
