...
For guidance on picking a tool when creating a digital human, see [Creating and configuring digital humans](/agents-create).
...
| **OpenClaw** | Open source | ✅ | ❌ | `.agent_context/skills/` (fallback) | Bound to the digital human, can't be switched per task |
...
Open-source project, a CLI digital human orchestrator. **Model is bound at the digital human layer** (`openclaw agents add --model`) — it can't be overridden per task. Configuration is strictly controlled: users can't pass `--model` or `--system-prompt`; the digital human-registration config decides.
...
**For your decision**: if your workflow needs digital humans to preserve context across tasks (failure retries, manual reruns, conversational iteration), pick only from the ✅ row.
...
If you set `mcp_config` in a digital human configuration but pick a tool other than Claude Code, your MCP servers have **no effect** on that digital human. MCP integration currently covers Claude Code only.
</Callout>
...
- [Creating and configuring digital humans](/agents-create) — pick a tool for your digital human
