...
创建数智人时挑选工具的指引见 [创建和配置数智人](/agents-create)。
...
| **OpenClaw** | 开源项目 | ✅ | ❌ | `.agent_context/skills/` （fallback）| 绑定在数智人上，不能在任务里切换 |
...
开源项目，CLI agent 编排器。**模型绑定在数智人层**（`openclaw agents add --model`）——不能在单次任务里覆盖。配置严格受控：用户不能传 `--model` 或 `--system-prompt`，由数智人注册时的配置决定。
...
**对你的决策**：如果工作流需要数智人在多次任务之间保持上下文（失败重试、手动重跑、对话式迭代），只选 ✅ 那一行的工具。
...
如果你在数智人配置里设置了 `mcp_config`，但选了 Claude Code 之外的工具，你的 MCP server 对这个数智人**没有效果**。目前的 MCP 集成只覆盖 Claude Code。
</Callout>
...
- [创建和配置数智人](/agents-create) —— 给你的数智人挑一款工具
