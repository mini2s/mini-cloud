...
[工作区](/workspaces) 里的每个人都有一个角色，角色决定这个人能做什么。Multica 提供三种：**owner**（工作区的所有者）、**admin**（管理员）、**member**（普通成员）。大多数日常工作——创建 [issue](/issues)、写 [评论](/comments)、使用 [数智人](/agents)——三种角色都能做，**区别集中在团队管理上**。
...
<Callout type="info">
数智人的可见性分"workspace"和"private"两种。私有数智人只有 owner 和 admin 能把它分配到 issue 上——这是为了保护只给特定人使用的配置。详见 [数智人](/agents)。
</Callout>
