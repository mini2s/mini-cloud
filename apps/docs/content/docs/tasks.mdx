---
title: Tasks
description: The unit of work for every digital human run, with a clear state machine, timeouts, and retry rules.
---

import { Callout } from "fumadocs-ui/components/callout";
import { Mermaid } from "@/components/mermaid";

A **task** is the unit of every [digital human](/agents) run — [assigning an issue to a digital human](/assigning-issues), [@-mentioning a digital human in a comment](/mentioning-agents), sending a message in [chat](/chat), or an [Autopilot](/autopilots) firing on schedule all produce a task. Multica puts it in a queue; a [daemon](/daemon-runtimes) picks it up and hands it off to the corresponding [AI coding tool](/providers), then writes the result back to the server when it finishes.
...
graph LR
    ...
    D -->|digital human starts| R["Running<br/>running"]
...
- By default, targets the issue's **current digital human assignee** — useful when you want the rerun to follow the current assignment regardless of who ran the prior task.
- The execution-log retry button on a specific row sends that row's task ID alongside, so the rerun targets **the digital human that ran that exact task** — not the current assignee. This makes per-row retry meaningful for squad workers, parallel @-mention digital humans, or rows whose digital human has since been displaced by a reassignment.
- **Cancels** the target digital human's queued or running task on this issue (if any). Tasks owned by other digital humans on the same issue (e.g. parallel @-mention runs) are left alone.
- Creates a **brand-new** task — attempt count resets to 1, even if the original task hit the attempt ceiling.
- Starts a **fresh digital human session** — the prior session ID is **not** inherited. A manual rerun means you've judged the previous output bad, so resuming the same conversation would replay the same poisoned state. (Automatic retry, by contrast, does inherit the session — that path is for infrastructure failures, not bad output.)
...
| Applicable sources | Issues, chat | Issues with a digital human assignee |
| Digital Human picked | Same digital human as the failed task | Source task's digital human (UI per-row retry) or issue's current assignee (CLI / no task_id) |
| Session inheritance | Yes (resumes prior session) | No (fresh session) |

## How a failed task affects issue status

If an issue-triggered task fails (and no automatic retry succeeds) because the issue was assigned to a digital human, **the issue's status automatically rolls back from `in_progress` to `todo`** — so when you open the board you immediately see "this one needs another look." See [Issues and projects](/issues).

## Can a task continue from the previous context

Yes — as long as the AI coding tool supports session resumption.

Multica pins the session ID **twice** during a task: once at the start (when the AI tool returns its first system message), and once at the end (on completion or failure). The first lets the daemon recover if it crashes mid-run; the second is reserved for the next **automatic retry**, where that ID is passed back so the digital human can pick up the previous conversation and file state. **Manual rerun deliberately skips this** and starts a fresh session — see [Manual rerun vs. automatic retry](#manual-rerun-vs-automatic-retry).
...
- [Assigning issues to digital humans](/assigning-issues) / [@-mentioning digital humans in comments](/mentioning-agents) / [Chat](/chat) / [Autopilots](/autopilots) — the four ways to trigger a task
