...
`issue` / `skill` / `task` 是 Multica 的核心实体。schema 字段、API 字段、路由和代码标识保持英文。中文 UI 中，面向用户的 `issue` 统一显示为「任务」；英文 UI 仍显示为 `Issue` / `issue`。其他实体中文里采用**混合规则** —— 词出现在哪里决定怎么写：

| 场景 | 写法 | 例 |
| --- | --- | --- |
| **中文 UI 短句中的 issue** | 中文术语 | "新建任务"、"创建子任务"、"任务列表" |
| **状态名 / 代码上下文 / API 或路由引用** | 小写英文 | "`issue_status`"、"`/{slug}/issues`"、"`task_id`" |
| **其他实体的 UI 短句** | 小写英文 | "排队中的 task"、"为数智人注入 skill" |
| **doc 标题 / 章节标题** | 首字母大写英文，**或**对应中文术语 | "Issue 与 project"、"Skills"、"执行任务" |
| **doc 正文长篇讨论中作为主语** | 中文术语，首次出现配括号英文 | "**执行任务**（task）是数智人每一次工作的单位" |
| **API / DB 字段** | 永远 `task` / `issue` / `skill` | `task_id`、`issue_status`、`skill_uuid` |

中文术语对照：

- `task` ↔ `执行任务`（上下文清楚后可简写为「任务」）
- `issue` ↔ 中文 UI 用 `任务`；英文 UI 和代码/API/路由上下文保留 `Issue` / `issue`
- `skill` 没有公认中文译法 —— 保留英文；标题可大写为 `Skills`

**为什么中文 UI 中 `issue` 改为「任务」，而代码上下文仍保留 `issue`**：

- **`issue`**：产品中文界面面向最终用户，统一叫「任务」更直白；英文界面仍使用 `Issue`，代码、API、DB、路由和 CLI 命令仍使用 `issue`，避免把实体名和技术契约混在一起改。
- **`task`**：表示数智人的执行单元，正式写法是「执行任务」，上下文清楚时可简称「任务」。当同一界面同时出现 issue 和 task 时，优先通过上下文或更具体的短语区分。
- **`skill`**：Multica 特有概念，没有公认中文译法，保留英文。
