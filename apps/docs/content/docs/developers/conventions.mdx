...
`issue` / `skill` / `task` are Multica's core entities. Schema fields, API fields, routes, and code identifiers stay English. In Chinese UI, user-facing `issue` copy is rendered as `任务`; the English UI still uses `Issue` / `issue`. Other entities follow a **mixed rule** — what to use depends on where the word appears:

| Context | Render | Example |
| --- | --- | --- |
| **Chinese UI strings for issue** | Chinese term | "新建任务"、"创建子任务"、"任务列表" |
| **State names, code references, API fields, routes** | lowercase English | "`issue_status`", "`/{slug}/issues`", "`task_id`" |
| **UI strings for other entities** | lowercase English | "排队中的 task"、"为数智人注入 skill" |
| **Doc titles / section headings** | Title-case English **or** the Chinese term | "Issue 与 project"、"Skills"、"执行任务" |
| **Long-form doc prose, when the entity is the running subject** | Chinese term, with English in parentheses on first mention | "**执行任务**（task）是数智人每一次工作的单位" |
...
- `issue` ↔ Chinese UI uses `任务`; English UI and code/API/route contexts keep `Issue` / `issue`
- `skill` has no settled Chinese translation — leave English; titles may capitalize as `Skills`

**Why Chinese UI renders `issue` as `任务`, while technical contexts keep `issue`**:

- **`issue`**: the Chinese product UI is user-facing, and `任务` is clearer there. The English UI still says `Issue`; code, API, DB, routes, and CLI commands still say `issue` so the product term and technical contract do not drift.
- **`task`**: means the execution unit performed by a digital human. The formal Chinese term is `执行任务`, shortened to `任务` when context is clear. When issue and task appear together, prefer context or a more specific phrase to disambiguate.
...
- "Assign to digital human" → "分配给数智人"
...
- `{{count}} digital humans` → `{{count}} 个数智人`
