...
- **Handlers** (`internal/handler/`): One file per domain (issue, comment, digital human, auth, daemon)
...
- **Digital Human SDK** (`pkg/agent/`): Unified `Backend` interface for executing prompts via Claude Code or Codex
