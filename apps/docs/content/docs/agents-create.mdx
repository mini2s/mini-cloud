---
title: Create and configure a digital human
description: The minimum fields to create a digital human, plus every optional setting — system instructions, environment variables, visibility, concurrency limit, and archiving.
---

import { Callout } from "fumadocs-ui/components/callout";

Creating an [digital human](/agents) takes only two things: **a name** and **a choice of [AI coding tool](/providers)**. Everything else is optional — system instructions, model, environment variables, CLI arguments, visibility, concurrency limit — the defaults work fine. Get it running first and tune later; every field can be changed at any time.
...
Once that's in place, go to the **Digital Humans** page in your workspace and click **+ New**, or use the CLI:
...
**System instructions** (`instructions`) are prepended to every task, telling the digital human what role it plays and what rules to follow:
...
When left blank (the default), the digital human uses the native behavior of its underlying AI coding tool with no extra constraints.
...
**Don't put high-value secrets in `custom_env`** (production database passwords, root-level tokens, etc.). Use **dedicated, limited-scope credentials** for digital humans (read-only API keys, single-scope PATs), and rotate them regularly.
</Callout>
...
- **Private** (`private`) — only workspace owners, admins, or the digital human's creator can assign it

New digital humans default to `private`.

**Private does not mean hidden** — every member sees a private digital human's name and description in the list, they just can't see sensitive config fields (the values in `custom_env` and MCP config are masked). Full meaning in [Digital Humans → Who can assign a digital human](/agents#who-can-assign-an-agent).

## Concurrency limit

**Concurrency limit** (`max_concurrent_tasks`) controls how many tasks this digital human can run in parallel at once. The default is **6**. New tasks that hit the cap queue up — they aren't rejected.

This is only the "digital human layer" of a two-tier limit — the daemon itself enforces a broader cap (default 20), and whichever is tighter wins. Details in [Daemon and runtimes → How many tasks can run in parallel](/daemon-runtimes#how-many-tasks-can-run-in-parallel).
...
A created digital human can have **Skills** attached — **knowledge packs** (`SKILL.md` + supporting files) automatically delivered to the AI coding tool at task execution time. You can create a new skill, import from GitHub or ClawHub, or scan one from an existing skill directory on your machine. See [Skills](/skills).

## Archive and restore

Digital Humans you no longer use can be **archived** — they disappear from everyday views, but their historical data (tasks run, comments posted) is fully preserved. **Restore** them anytime to put them back to work.

<Callout type="warning">
**Archiving immediately cancels every unfinished task belonging to the digital human** — running, dispatched, and queued tasks are all marked `cancelled` and won't continue. If you have an important task in flight, let it finish before archiving.
</Callout>

Archived digital humans can't be assigned new tasks.
...
- [Assigning issues to digital humans](/assigning-issues) — put your new digital human to work
