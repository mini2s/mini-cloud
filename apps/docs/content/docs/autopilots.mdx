...
- **Name** — display name
- **Digital Human** — who the run is dispatched to
- **Priority** — inherited by the `task` it produces (same semantics as issue priority)
- **Description / prompt** — the work description the digital human receives each run
...
stores the body as the run's `trigger_payload`, and dispatches the digital human
...
description so the digital human can read it inline. In **run-only mode**, the
payload is part of the run context the daemon hands the digital human.
...
If an autopilot is important, design your own monitoring — for example, have the digital human post a comment on success, and catch failures by noticing missing comments.
</Callout>
...
- [**Assign issues to digital humans**](/assigning-issues) — a one-shot hand-off of an issue to a digital human
- [**@-mention digital humans in comments**](/mentioning-agents) — pull a digital human in to take a look from a comment
