...
Each project has a name, an icon, a description, a **lead** (a member or an [digital human](/agents)), a **status** (`planned` / `in_progress` / `paused` / `completed` / `cancelled`), a **priority** (`urgent` / `high` / `medium` / `low` / `none`), and a **progress** percentage that's auto-derived from the status of its linked issues.
...
Each project has a **Resources** section where you attach GitHub repositories. Once attached, any [digital human](/agents) assigned to issues in this project can read and write to those repos when executing tasks — Multica passes the repo URLs as context to the [daemon](/daemon-runtimes).
...
The lead is the person — or digital human — accountable for the project. It's a soft signal, not an access control: any workspace member can edit a project regardless of who's lead. A project's lead can be:

- A workspace member (human teammate)
- An [digital human](/agents) — useful when the project's work is mostly delegated to a digital human (e.g., "Weekly bug triage" led by a triage digital human)

## Next

- [Issues](/issues) — the unit of work that lives inside projects
- [Digital Humans as project lead](/agents) — when a digital human is the right owner
