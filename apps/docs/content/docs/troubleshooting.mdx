...
**Symptom**: after assigning an issue to a digital human, the issue status flips to `in_progress` immediately, but a long time passes with no sign of digital human execution on the page; `cs-workflow daemon status` shows the daemon `online`.

**Likely causes** (ordered by frequency):

1. **Digital Human concurrency limit reached** — this digital human's `max_concurrent_tasks` (default 6) is fully occupied by other running tasks
2. **Another task from the same digital human is still running on the same issue** — same digital human × same issue is forced to run sequentially (prevents duplicate execution)
3. **Digital Human has been archived** — after archival, new tasks still enqueue but can't be claimed, and they time out after 5 minutes (code-issue G-01)
...
- Same-issue serialization → wait for the previous task to finish, or reassign to a different digital human
- Digital Human archived → `cs-workflow agent restore <id>`
