...
本计划验证工作流内置 Agent 在真实系统链路中的节点级运行时选择行为：

```text
人工指定 -> 空闲运行时 -> 所属 Issue 创建人的运行时 -> 无可用则失败
```

必须证明以下结果：

1. 选择发生在每个 Worker/Critic 阶段派发时，而不是整次工作流只选择一次。
2. 人工偏好有效时始终优先，即使该运行时繁忙。
3. 自动模式优先使用工作区内合格的空闲运行时。
4. 没有空闲运行时时，只允许回退到所属 Issue 创建人拥有的运行时。
5. Issue 由 Agent 创建时，责任人正确解析为该 Agent 的 Owner。
...
10. 所有交互式工作流启动入口始终显示运行时选择器，默认“自动选择”。
...
| --- | --- | --- | --- | --- | --- |
| R-MANUAL | W-MAIN | U-OPERATOR | online/healthy | public | 人工指定 |
...
| R-CREATOR-2 | W-MAIN | U-CREATOR | online/healthy | private | 验证责任人最小负载排序 |
...
SELECT
    wr.id AS run_id,
    wr.runtime_id AS manual_preference,
    wr.source_issue_id,
...
- `workflow_run.runtime_id` 只表示人工偏好，可以为 NULL。
...
| ID | 场景与步骤 | 预期结果 |
| --- | --- | --- |
| UI-01 | W-MAIN 没有在线运行时；创建或改派 Issue 为 WF-WORKER | 仍显示选择器；默认选中“自动选择”；可以确认 |
| UI-02 | 只有 R-IDLE-1 在线；选择 WF-WORKER | 仍显示选择器，不自动跳过；包含“自动选择”和 R-IDLE-1 |
| UI-03 | R-IDLE-1、R-IDLE-2 在线 | 显示两个运行时和“自动选择” |
| UI-04 | 选择不含内置 Agent 的 WF-NO-BUILTIN | 仍显示选择器 |
| UI-05 | 在工作流编辑器点击“保存并测试” | 先完成保存，再显示选择器；确认前不发送启动请求 |
| UI-06 | 保持默认“自动选择”并确认 | 请求不携带 `runtime_id` 或值为 null；运行正常创建 |
| UI-07 | 人工选择 R-MANUAL 并确认 | 请求携带 `runtime_id=R-MANUAL`；run 保存该偏好 |
| UI-08 | R-OFFLINE 存在 | 离线运行时不出现在人工选项中 |
| UI-09 | 关闭选择器 | 不创建/改派 Issue，不启动测试运行 |
| UI-10 | WF-BOUND 人工选择 R-MANUAL 后运行 | 选择器说明普通 Agent 保持自身绑定；实际 task 使用 R-BOUND |
...
| API-02 | 人工指定 R-CROSS | HTTP 400，不创建 run/task |
...
| API-06 | 缺少 `runtime_id` | 与自动选择完全等价 |
| API-07 | `runtime_id` 为空字符串或 null | 与自动选择完全等价，不发生 UUID panic |
...
| --- | --- | --- | --- |
| SEL-01 | R-MANUAL 有 3 个活动 task，R-IDLE-1 空闲；人工指定 R-MANUAL | R-MANUAL | manual |
| SEL-02 | 人工偏好 R-OFFLINE，R-IDLE-1 空闲；在偏好保存后使其离线再派发 | R-IDLE-1 | idle |
| SEL-03 | 人工偏好 R-STALE，R-IDLE-1 空闲 | R-IDLE-1 | idle |
| SEL-04 | 自动模式；R-IDLE-1、R-IDLE-2 均空闲，R-IDLE-1 心跳更新 | R-IDLE-1 | idle |
| SEL-05 | 自动模式；所有非责任运行时繁忙；R-CREATOR-1 负载 2，R-CREATOR-2 负载 1 | R-CREATOR-2 | issue_creator |
| SEL-06 | R-CREATOR-1 繁忙，但 R-IDLE-1 空闲 | R-IDLE-1 | idle |
| SEL-07 | 没有空闲运行时；只有 R-OTHER-BUSY 合格且繁忙，责任人没有运行时 | 无 | runtime_unavailable |
...
| CON-02 | 同时启动 10 次 WF-WORKER；提供 3 个空闲运行时 | 前 3 个选择不会重复判断同一运行时为空闲；后续结果遵循责任人/失败规则 |
...
| PHASE-02 | 人工指定 R-MANUAL，依次执行 Worker/Critic | 两个阶段都重新校验，但偏好持续合格时都使用 R-MANUAL，reason=`manual` |
| RETRY-01 | Workflow task 因 `runtime_offline` 失败并触发自动重试；原运行时已离线，另一运行时空闲 | retry child 使用新运行时；节点关联更新；parent_task_id 正确 |
| RETRY-02 | 自动重试时人工偏好仍有效但繁忙 | retry child 仍使用人工偏好，reason=`manual` |
...
| ID | 前置条件与步骤 | 预期结果 |
| --- | --- | --- |
| FAIL-01 | 自动模式、无所属 Issue、无空闲运行时 | 当前节点 failed，run failed，reason=`runtime_unavailable` |
| FAIL-02 | WF-PARALLEL 中一个节点已 running，另一个节点选择失败 | running task 收到取消并变为 cancelled；兄弟节点 cancelled |
| FAIL-03 | 人工偏好失效、无空闲、Issue 创建人没有合格运行时 | 完整回退后 fail-fast，不无限等待 |
...
| SNAP-03 | run 创建后变更 A-CREATOR.owner_id | 既有 run 的责任人快照不变 |
...
| COMP-03 | 新后端接收不带 `runtime_id` 的旧客户端请求 | 自动模式正常启动 |
...
- Web 与 Desktop 各执行一次启动、自动选择和人工指定 smoke。
