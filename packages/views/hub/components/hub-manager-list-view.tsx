    ...
    >
      ...
      <div className="relative z-[1] flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          {/* Type chip */}
          <span
            className="inline-flex h-5.5 shrink-0 items-center whitespace-nowrap rounded-full px-2 text-[11px] font-bold"
            style={{
...
function BatchBar({
  ...
  return (
    <div className="mb-2.5 flex w-full flex-wrap items-center gap-x-4 gap-y-2 rounded-xl bg-[color:color-mix(in_oklab,var(--primary)_8%,var(--background))] px-4 py-2.5 ring-1 ring-inset ring-[color:color-mix(in_oklab,var(--primary)_18%,transparent)] shadow-[0_6px_16px_-8px_color-mix(in_oklab,var(--primary)_45%,transparent)]">
      ...
      <button
        type="button"
        onClick={onClear}
        className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-border/40 px-3 text-xs font-semibold text-muted-foreground transition-colors hover:bg-accent"
      >
      ...
      <button
        type="button"
        onClick={onDelete}
        className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-red-400/40 bg-red-500/10 px-3 text-xs font-bold text-red-500 transition-colors hover:bg-red-500/20 hover:text-red-600"
      >
        <Trash2 size={14} />
...
function ErrorState({ msg, onRetry }: { msg: string; onRetry?: () => void }) {
  return (
    <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border/30 px-6 py-14 text-center">
      <span className="text-sm font-semibold text-red-500">{msg}</span>
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-border/40 px-3 text-xs font-semibold transition-colors hover:bg-accent"
        >
          {"Retry"}
        </button>
      )}
...
export function HubManagerListView({
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Select-all header */}
      {selectable && (
        <div className="flex items-center gap-3.5 border-b border-border/20 px-4 py-2">
          <button
            type="button"
            role="checkbox"
            ...
            className={cn(
              ...
              allSelected
                ? "border-[var(--primary)] bg-[var(--primary)] text-white"
                : someSelected
                  ? "border-[var(--primary)] bg-[var(--primary)]/20 text-primary"
                  : "border-border/50 text-transparent hover:border-[var(--primary)]",
            )}
          >
