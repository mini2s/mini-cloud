  ...
  const nodeStatuses = useMemo(() => {
    ...
    for (const node of nodes) {
      ...
      if (nodeRun) {
        const status = nodeRun.status as NodeRunStatus;
        statuses[node.id] = {
          status: t(($) => ($.node_run.status as Record<string, string>)[status] ?? status),
    ...
    <div className="flex h-full flex-col">
      <PageHeader className="shrink-0 justify-between px-5">
        <div className="flex min-w-0 items-center gap-2">
          <h1 className="truncate text-sm font-medium">{run.workflow_title}</h1>
          <Badge variant="secondary" className="h-4 px-1.5 text-[10px]">
            {t(($) => ($.run.status as Record<string, string>)[run.status as WorkflowRunStatus] ?? run.status)}
          </Badge>
        </div>
        {canCancel ? (
          <Button size="sm" variant="outline" onClick={() => cancelMutation.mutate({ workflowId, runId })} disabled={cancelMutation.isPending}>
      ...
      <div className="flex min-h-0 flex-1">
        <div className="flex-1 bg-muted/20">
          {nodes.length > 0 ? (
            <ReactFlowProvider>
              <DAGCanvas nodes={nodes} edges={edges} nodeStatusColors={nodeStatusColors} nodeStatuses={nodeStatuses} />
            </ReactFlowProvider>
          ) : (
        ...
        <aside className="w-96 shrink-0 space-y-4 overflow-y-auto border-l bg-card p-3">
          ...
          <section className="space-y-2">
            ...
            {nodeRuns.map((nodeRun) => <NodeRunCard key={nodeRun.id} nodeRun={nodeRun} maxRetries={3} workflowId={workflowId} runId={runId} />)}
          </section>
        </aside>
      </div>
    </div>
