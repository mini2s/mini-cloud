...
// EfficiencyDimension integration smoke test. Mirrors the cost-kanban.test
// pattern: mock the workspace hook + view-state, and intercept useQuery to
// return the real mock-data factories keyed off the queryKey shape so the
// aggregate view exercises its "data present" render path. The whole point
// is that the page graph mounts and the four entity tabs + default (org)
// dept ranking + KPI strip render without throwing — the most faithful check
...
vi.mock("@tanstack/react-query", async () => {
  ...
  return {
    ...
    useQuery: (opts: { queryKey: unknown[] }) => {
      ...
      // dept-tree carries no date params; everything else carries them at
      // key[3..4] (or key[4..5] for dept-ranking whose key[3] is parentDeptId).
      ...
      } else if (segment === "efficiency-aggregate") {
        data = eff.mock.efficiencyAggregate({ startDate: start, endDate: end });
      ...
      } else if (segment === "all-needs") {
        data = eff.mock.allNeeds({ startDate: start, endDate: end });
...
import { EfficiencyDimension } from "./efficiency-dimension";

describe("EfficiencyDimension — full page integration render", () => {
  beforeEach(() => {
    cleanup();
  });

  it("mounts without throwing and renders the page header title", () => {
    renderWithI18n(<EfficiencyDimension />);
  ...
  it("renders the aggregate overview/distribution sub-tabs", () => {
    renderWithI18n(<EfficiencyDimension />);
  ...
  it("renders the org ranking table header by default (org is the landing entity)", () => {
    renderWithI18n(<EfficiencyDimension />);
    expect(screen.getByText("部门效率排行")).toBeInTheDocument();
  });
});
