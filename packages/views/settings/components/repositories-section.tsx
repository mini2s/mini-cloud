...
export function RepositoriesSection({ host }: RepositoriesSectionProps) {
  ...
  // Each RepositoriesSection instance holds an independent copy of the full
  // repos array. Safe because the settings page renders only one section at a
  // time (Radix Tabs unmounts inactive tab content); if two ever render
  // simultaneously, their states must be lifted/synchronized to avoid one
  // overwriting the other's unsaved edits.
  const [repos, setRepos] = useState<WorkspaceRepo[]>(allRepos);
  const [editingIndices, setEditingIndices] = useState<Set<number>>(new Set());
  /** Indices of rows that were added in this section (not yet persisted). Kept
   *  visible regardless of repoHost so the user can finish typing before save. */
  const [localIndices, setLocalIndices] = useState<Set<number>>(new Set());
  ...
  useEffect(() => {
    setRepos(allRepos);
    setLocalIndices(new Set());
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [workspace]);

  // Visible slice: entries whose host bucket matches this section, OR rows
  // that were locally added in this section and haven't been saved yet.
  const visible = repos
    .map((r, index) => ({ r, index }))
    .filter(({ r, index }) => localIndices.has(index) || repoHost(r.url) === host);

  // dirty = local edits produced a new array reference (setRepos always returns a new array).
  ...
  const handleAddRepo = () => {
    ...
    setLocalIndices(new Set(localIndices).add(nextIndex));
  };

  const handleRemoveRepo = (index: number) => {
    setRepos(repos.filter((_, i) => i !== index));
    setEditingIndices(dropAndShiftIndex(editingIndices, index));
    setLocalIndices(dropAndShiftIndex(localIndices, index));
  };
