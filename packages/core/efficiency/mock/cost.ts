...
export function getMockCostOverview(q: DeptQuery): CostOverviewResp {
  ...
  return {
    ...
    input_cost_pct: totalCost > 0 ? (inputCost / totalCost) * 100 : 0,
    output_cost_pct: totalCost > 0 ? (outputCost / totalCost) * 100 : 0,
...
export function getMockCostModelComposition(
  ...
  const items: CostCompositionItem[] = [
    { model: MODEL_NAMES[0]!, total_cost: 412.6, cost_pct: 46.35 },
    { model: MODEL_NAMES[1]!, total_cost: 218.4, cost_pct: 24.54 },
    { model: MODEL_NAMES[2]!, total_cost: 162.8, cost_pct: 18.29 },
    { model: MODEL_NAMES[3]!, total_cost: 96.2, cost_pct: 10.81 },
  ];
  return { items };
}
